...
from apps.worker.models import CustomUser_Funcionario  # Use o modelo de usuário personalizado
...
class RegisterForm(forms.Form):
    ...
    def clean_telefone(self):
        ...
        if telefone and len(str(telefone)) < 10:
            raise forms.ValidationError("O número de telefone deve conter pelo menos 10 dígitos.")
        return telefone
    ...
    def clean_email(self):
        ...
        if CustomUser_Funcionario.objects.filter(email=email).exists():  # Use o modelo de usuário personalizado
            raise ValidationError("Este e-mail já está registrado.")
        return email

    def clean_log(self):
        log = self.cleaned_data.get("log")
        if CustomUser_Funcionario.objects.filter(username=log).exists():  # Use o modelo de usuário personalizado
